...
class StudentImportService
{
    ...
    public function import(string $path, string $extension, int $schoolId, Collection $classes): array
    {
        if (strtolower($extension) === 'xlsx' && ! class_exists(ZipArchive::class)) {
            return [
                'created' => 0,
                'skipped' => [],
                'error' => 'XLSX import requires the PHP Zip extension. Please upload a CSV file instead.',
            ];
        }
        ...
        if (empty($rows)) {
            return ['created' => 0, 'skipped' => [], 'error' => 'No student records found in the uploaded file.'];
        }
        ...
        foreach ($rows as $index => $row) {
            ...
            if (! $classId) {
                $skipped[] = "Row {$line}: Class not found or not assigned.";
                continue;
            }
    ...
    private function validateImportHeaders(array $headers): ?string
    {
        ...
        $allowed = array_merge(self::HEADERS, ['class_name', 'class_id']);
    ...
    private function resolveClassId(array $row, Collection $classes): ?int
    {
