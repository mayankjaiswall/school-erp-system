...
@section('content')
<style>.child-card{background:#fff;border:1px solid #e2e8f0;border-radius:18px;padding:24px;box-shadow:0 8px 20px rgba(15,23,42,.05);height:100%;transition:.2s ease}.child-card:hover{transform:translateY(-2px);box-shadow:0 12px 28px rgba(15,23,42,.08)}.child-card-link{color:inherit;text-decoration:none;display:block;height:100%}.child-photo{width:92px;height:92px;border-radius:50%;object-fit:cover;border:4px solid #dbeafe}</style>
<div class="row g-4">
    @forelse($children as $child)
        <div class="col-lg-6">
            <a class="child-card-link" href="{{ route('parent.children.show', $child->id) }}">
