...
<div class="content-card mt-0">
    <div class="row g-3 align-items-end">
        ...
        <div class="col-md-5"><label class="form-label fw-semibold">Exam</label><select id="exam_id" class="form-select"><option value="">All Exams</option>@foreach($exams as $exam)<option value="{{ $exam->id }}">{{ $exam->name }} - {{ $exam->academic_year }}</option>@endforeach</select></div>
...
$(function () {
    ...
    function loadResults() {
        $.getJSON(url, {student_id: $('#student_id').val(), exam_id: $('#exam_id').val()})
            .done(function (response) {
                ...
                $('#resultsBody').html(response.results.map((result) => `
                    <div class="mb-4">
                        ...
                        <table class="table align-middle"><thead><tr><th>Subject</th><th>Marks</th><th>Percentage</th><th>Grade</th><th>Remarks</th></tr></thead><tbody>
                            ${result.subjects.map((subject) => `<tr><td>${escapeHtml(subject.subject)}</td><td>${escapeHtml(subject.marks_obtained)} / ${escapeHtml(subject.max_marks)}</td><td>${escapeHtml(subject.percentage)}%</td><td>${escapeHtml(subject.grade)}</td><td>${escapeHtml(subject.remarks || '-')}</td></tr>`).join('')}
                        </tbody><tfoot><tr><th>Total</th><th>${escapeHtml(result.obtained_marks)} / ${escapeHtml(result.total_marks)}</th><th>${escapeHtml(result.percentage)}%</th><th>${escapeHtml(result.grade)}</th><th>Attendance: ${escapeHtml(result.attendance_percentage)}%</th></tr></tfoot></table>
                    </div>`).join(''));
            })
            .fail(function () { Swal.fire({icon:'error', title:'Unable to load results', text:'Please try again.'}); });
    }
    $('#loadResults, #student_id, #exam_id').on('click change', loadResults);
    loadResults();
});
</script>
@endpush
