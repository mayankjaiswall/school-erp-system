...
@section('content')
<style>
    .report-toolbar{background:#fff;border:1px solid #e5e7eb;border-radius:18px;box-shadow:0 10px 30px rgba(15,23,42,.08);padding:24px}
...
<div class="mt-4" id="reportCardContainer">
    <div class="empty-report">Select exam and assigned class to load report cards.</div>
</div>
@endsection
...
<script>
$(function () {
    const studentsUrl = "{{ route('teacher.report-cards.students') }}";
    const generateUrl = "{{ route('teacher.report-cards.generate') }}";
    const printUrl = "{{ route('teacher.report-cards.print') }}";
    const downloadUrl = "{{ route('teacher.report-cards.download-pdf') }}";
    ...
    function loadStudents() {
        ...
        $('#student_id').html('<option value="">All assigned students</option>').prop('disabled', true);
        setActionState(false);

        if (!classId) {
            $('#reportCardContainer').html('<div class="empty-report">Select exam and assigned class to load report cards.</div>');
            return;
        }
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#student_id').html('<option value="">All assigned students</option>' + options).prop('disabled', false);
                loadReportCards();
            },
            error: function (xhr) {
                alertError('Unable to load students', xhr, 'Please choose one of your assigned classes.');
            }
        });
    }
    ...
    function validateSelection() {
        if (!$('#exam_id').val() || !$('#class_id').val()) {
            Swal.fire({icon:'warning', title:'Selection required', text:'Select exam and assigned class first.'});
            return false;
        }
